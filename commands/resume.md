...
Resume a previous scan run — switch the active run back to it so `/report`, `/fix`, and
`/status` operate on its findings again. **No re-scanning**: it reuses the findings already
...
> project and running `/report` or `/fix` already continues your last run. Use `resume` to switch
> *back* to an earlier run — find its ID with `/lazy-refactor:list`.
...
- `<id>` (required): The run ID to resume. Get it from `/lazy-refactor:list`.
...
1. **Switch the active run**: Call `set_active_run` with the given `id`. This is instant — it
   ...
   - If the id doesn't exist, report the error and suggest `/lazy-refactor:list` to find a valid
     id. Do not create or guess a run.

2. **Confirm and orient**: After switching, show the run's current summary (total findings,
   breakdown by severity and status) — the same shape as `/lazy-refactor:status` — so the user can
   see what they're back in.

3. **Suggest next actions**: Point to `/lazy-refactor:report` to review and `/lazy-refactor:fix`
   to apply fixes against the now-active run.

> Need fresh findings because the code has changed? Run `/lazy-refactor:scan` — that's a new scan,
> deliberately separate from resuming an existing one.

> **Don't confuse `/resume` with the `resume_scan` MCP tool.** Despite the similar name, `resume_scan`
> is a *different* operation that re-activates a run **and re-runs detection into it** (a re-scan);
> this `/resume` command deliberately uses `set_active_run` and does **not** re-scan.
