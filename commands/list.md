...
report on, or fix. Each `/lazy-refactor:scan` creates a new **run** (its own findings
...
2. **Display a table**, one row per run:
   - **Run ID** — the id used by `/lazy-refactor:resume <id>`
   - **Active** — mark the currently active run (e.g. `●`); this is the run `/report`,
     `/fix`, and `/status` operate on
...
3. **Handle no runs**: If `list_runs` returns an empty list, tell the user no scans exist
   yet and suggest `/lazy-refactor:scan`.

4. **Suggest next actions**: Remind the user they can:
   - `/lazy-refactor:resume <id>` — switch to a run and continue where they left off
   - `/lazy-refactor:report` / `/lazy-refactor:fix` — these act on the active run
